...
class OpenMeteoProvider {
	hourlyParams = [
		...
		"windgusts_10m", "shortwave_radiation", "direct_radiation", "direct_normal_irradiance",
		"diffuse_radiation", "vapor_pressure_deficit", "cape", "evapotranspiration",
		"et0_fao_evapotranspiration", "precipitation", "snowfall", "precipitation_probability",
		"rain", "showers", "weathercode", "snow_depth", "freezinglevel_height", "visibility",
		"soil_temperature_0cm", "soil_temperature_6cm", "soil_temperature_18cm", "soil_temperature_54cm",
		"soil_moisture_0_1cm", "soil_moisture_1_3cm", "soil_moisture_3_9cm", "soil_moisture_9_27cm",
		"soil_moisture_27_81cm", "uv_index", "uv_index_clear_sky", "is_day",
		"terrestrial_radiation", "terrestrial_radiation_instant", "shortwave_radiation_instant",
		"diffuse_radiation_instant", "direct_radiation_instant", "direct_normal_irradiance_instant"
	];

	dailyParams = [
		"temperature_2m_max", "temperature_2m_min", "apparent_temperature_min",
		"apparent_temperature_max", "precipitation_sum", "rain_sum", "showers_sum",
		"snowfall_sum", "precipitation_hours", "weathercode", "sunrise", "sunset",
	...
	start () {
		this._doFetch();
		if (!this._timer) {
			this._timer = setInterval(() => this._doFetch(), this.config.updateInterval);
		}
	}
	...
	async _fetchLocation () {
		...
		try {
			...
			console.debug("[openmeteo] Could not load location data:", error.message);
		}
	}
	...
	async _doFetch () {
		...
		let response;
		try {
			response = await fetch(this._url, { headers });
		} catch (err) {
			if (this.onErrorCallback) {
				this.onErrorCallback({ message: err.message, translationKey: "MODULE_ERROR_UNSPECIFIED" });
			}
			return;
		}
		...
		if (!response.ok) {
			if (this.onErrorCallback) {
				this.onErrorCallback({ message: `HTTP ${response.status}`, translationKey: "MODULE_ERROR_UNSPECIFIED" });
			}
			return;
		}
		...
		try {
			...
			console.error("[openmeteo] Failed to parse JSON:", error);
			if (this.onErrorCallback) {
				this.onErrorCallback({ message: "Failed to parse API response", translationKey: "MODULE_ERROR_UNSPECIFIED" });
			}
		}
	}

	_handleResponse (data) {
		const parsedData = this._parseWeatherApiResponse(data);

		if (!parsedData) {
			if (this.onErrorCallback) {
				this.onErrorCallback({ message: "Invalid API response", translationKey: "MODULE_ERROR_UNSPECIFIED" });
			}
			return;
		}
		...
		try {
			...
			switch (this.config.type) {
				case "current":
					weatherData = this._generateWeatherDayFromCurrentWeather(parsedData);
					break;
				case "forecast":
				case "daily":
					weatherData = this._generateWeatherObjectsFromForecast(parsedData);
					break;
				case "hourly":
					weatherData = this._generateWeatherObjectsFromHourly(parsedData);
					break;
				default:
			...
			console.error("[openmeteo] Error processing weather data:", error);
			if (this.onErrorCallback) {
				this.onErrorCallback({ message: error.message, translationKey: "MODULE_ERROR_UNSPECIFIED" });
			}
		}
	}
	...
	_getQueryParameters () {
		...
		switch (this.config.type) {
			...
			case "daily":
			case "forecast":
				params.forecast_days = maxNumberOfDays + 1;
				break;
			case "current":
	...
	_getUrl () {
		return `${this.config.apiBase}/forecast?${this._getQueryParameters()}`;
	}

	_transposeDataMatrix (data) {
	...
	_parseWeatherApiResponse (data) {
		...
		if (type === "current" && !validByType.daily && !validByType.hourly) return null;

		for (const key of ["hourly", "daily"]) {
			if (typeof data[key] === "object") {
				data[key] = this._transposeDataMatrix(data[key]);
			}
		}
	...
	_convertWeatherType (weathercode, isDayTime) {
	...
	_isDayTime (date, sunrise, sunset) {
		const time = date.getTime();
		return time >= sunrise.getTime() && time < sunset.getTime();
	}

	_generateWeatherDayFromCurrentWeather (parsedData) {
		const current = {
			...
			weatherType: this._convertWeatherType(parsedData.current_weather.weathercode, true)
		};
		...
		if (parsedData.daily && Array.isArray(parsedData.daily) && parsedData.daily[0]) {
			...
			if (today.sunrise) current.sunrise = today.sunrise;
			if (today.sunset) {
				current.sunset = today.sunset;
				if (current.sunrise && current.sunset) {
					current.weatherType = this._convertWeatherType(
						parsedData.current_weather.weathercode,
						this._isDayTime(parsedData.current_weather.time, current.sunrise, current.sunset)
					);
				}
			}
			if (today.temperature_2m_min !== undefined) current.minTemperature = today.temperature_2m_min;
			if (today.temperature_2m_max !== undefined) current.maxTemperature = today.temperature_2m_max;
		}

		return current;
	}

	_generateWeatherObjectsFromForecast (parsedData) {
		return parsedData.daily.map((weather) => ({
			...
			weatherType: this._convertWeatherType(weather.weathercode, true),
	...
	_generateWeatherObjectsFromHourly (parsedData) {
		const hours = [];
		const now = new Date();

		parsedData.hourly.forEach((weather, i) => {
			if (weather.time <= now) return;
			...
			hours.push({
				...
				weatherType: this._convertWeatherType(
					weather.weathercode,
					this._isDayTime(weather.time, dailyData.sunrise, dailyData.sunset)
				),
